...
/// difference, and the second is the endgame difference.
/// 
/// # Panics
/// 
...
pub fn pst_delta(board: &Board, m: Move) -> (Eval, Eval) {
    ...
    let mut mg_delta = 
        MIDGAME_VALUE[end_idx][to_idx] 
        - MIDGAME_VALUE[mover_idx][from_idx];
    let mut eg_delta = 
        ENDGAME_VALUE[end_idx][to_idx] 
        - ENDGAME_VALUE[mover_type as usize][from_idx];
...
mod tests {
    ...
    fn test_pst_delta_equals_base_result() {
        ...
        for m in get_moves(&b) {
            ...
            let delta_eval = (
                pst_original.0 + delta.0,
                pst_original.1 + delta.1,
            );
            let mut bcopy = b;
            bcopy.make_move(m);
